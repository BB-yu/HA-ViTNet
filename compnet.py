...
T_MAX = 32768 #128*128 2048 均不可以 # increase this if your ctx_len is long [NOTE: TAKES LOTS OF VRAM!]
# it's possible to go beyond CUDA limitations if you slice the ctx and pass the hidden state in each slice

from torch.utils.cpp_extension import load


wkv_cuda = load(name="wkv", sources=[os.path.join(os.getcwd(), 'wkv_op.cpp'), os.path.join(os.getcwd(), 'wkv_cuda.cu')],
                verbose=True, extra_cuda_cflags=['-res-usage', '--maxrregcount 60', '--use_fast_math', '-O3', '-Xptxas -O3', f'-DTmax={T_MAX}'])


def DepthwiseConv2d(in_channels, out_channels, kernel_size, stride=1, padding=0, dilation=1, bias=False):
    return nn.Conv2d(
        in_channels,
        out_channels,
...
class VRWKV_Bottleneck(nn.Module):
    def __init__(self, n_embd, n_layer, layer_id, shift_mode='q_shift',
        ...
        """
        super().__init__()
        print(f"VRWKV_Bottleneck init: n_embd = {n_embd}")
        self.layer_id = layer_id
        self.n_embd = n_embd
...
class BinaryOrientatedRWKV2D(nn.Module):
    def __init__(self, n_embd, n_layer, shift_mode='q_shift', channel_gamma=1/4, shift_pixel=1, hidden_rate=4, init_mode='fancy', drop_path=0., k_norm=True):
        super().__init__()
        print(f"BinaryOrientatedRWKV2D init: n_embd = {n_embd}")
    ...
    def forward(self, z):
        B, C, H, W = z.shape
        rwkv_f = self.rwkv_forward(z)  # [B,C,H,W] → [B,C,H,W]
        z_rev = z.transpose(2, 3)      # [B,C,W,H]
        rwkv_r = self.rwkv_reverse(z_rev)  # [B,C,W,H]
        rwkv_r = rwkv_r.transpose(2, 3)    # back to [B,C,H,W]
        return rwkv_f + rwkv_r
...
class LoRA(nn.Module):
    def __init__(self, input_channel=3, num_classes=1, dims=[32, 64, 128, 256,768], depths=[1, 1, 1, 3, 1], kernel_sizes=[[3, 3]] * 5, expansion_factors=[4] * 5):
        ...
        self.n_emb = dims[-1]
        print(f"LoRA init: self.n_emb = {self.n_emb}") 
    ...
    def forward(self, x):
        ...
        x4, p4 = self.e4(p3)
        x5, p5 = self.e5(p4)
        print(f"Shape of x5 before Brwkv: {x5.shape}")
