...
class DecoderBlock(nn.Module):
    def __init__(
        ...
        use_batchnorm=True,
        # 新增一个参数，用于传递 compnet.py 中 RWKV 模块的 T_MAX
        # 这是非常重要的，你需要从你的compnet.py或其他配置中获取这个真实值
        # 否则，超过 T_MAX 的输入仍然会导致 Assertion Error
        rwkv_t_max: int = 32768, # 默认值，请务必根据你的实际 compnet.py 中的 T_MAX 进行设置！
    ):
        super().__init__()
        ...
        self.rwkv_t_max = rwkv_t_max # 存储 T_MAX 值

        # 核心修改：为 Pre-Norm 和 Post-Norm 分支分别创建 RWKV 模块及其归一化层
        if skip_channels > 0:
            # Pre-Norm 分支
            # LayerNorm 将应用于通道维度，通常在 Transformer Block 中用于序列建模
            # 这里为了处理 (B, C, H, W) 格式，我们会在 forward 中进行permute
            self.pre_norm_layer = nn.LayerNorm(skip_channels) 
            self.rwkv_pre_norm = BinaryOrientatedRWKV2D(
                n_embd=skip_channels, n_layer=12,
                shift_mode='q_shift', channel_gamma=1/4,
                shift_pixel=1, hidden_rate=4, init_mode="fancy",
                drop_path=0., k_norm=True
            )
            
            # Post-Norm 分支
            self.rwkv_post_norm = BinaryOrientatedRWKV2D(
                n_embd=skip_channels, n_layer=12,
                shift_mode='q_shift', channel_gamma=1/4,
                shift_pixel=1, hidden_rate=4, init_mode="fancy",
                drop_path=0., k_norm=True
            )
            self.post_norm_layer = nn.LayerNorm(skip_channels)
        else:
            # 如果没有 skip 连接，则不使用 RWKV 和归一化层
            self.pre_norm_layer = None
            self.rwkv_pre_norm = None
            self.rwkv_post_norm = None
    ...
    def forward(self, x, skip=None):
        ...
        if skip is not None:
            # --- 1. 空间对齐 skip 和 x ---
            # 无论是否应用 RWKV，都需要确保 skip 和 x 具有相同的空间维度
            if x.shape[2:] != skip.shape[2:]:
                # print(f"Warning: Spatial dimensions of x ({x.shape[2:]}) and skip ({skip.shape[2:]}) mismatch. Resizing skip.")
                diff_y = skip.size()[2] - x.size()[2]
                diff_x = skip.size()[3] - x.size()[3]
                # 裁剪 skip 到与 x 相同的尺寸
                skip_aligned = skip[:, :, diff_y // 2:diff_y // 2 + x.size()[2],
                                          diff_x // 2:diff_x // 2 + x.size()[3]]
            else:
                skip_aligned = skip # 如果已经对齐，直接使用

            # --- 2. 检查 RWKV 模块应用条件 ---
            current_T = skip_aligned.shape[2] * skip_aligned.shape[3]
            
            # 只有当 RWKV 模块存在且当前 T 值在可接受范围内时，才进行 RWKV 处理
            if self.rwkv_pre_norm is not None and current_T <= self.rwkv_t_max:
                # print(f"Applying Pre/Post-Norm RWKV to skip of shape {skip_aligned.shape} (T={current_T}, T_MAX={self.rwkv_t_max})")

                # --- 3. Pre-Norm 分支处理 ---
                # Pre-Norm: 先归一化，再输入到 RWKV
                # LayerNorm 默认对最后一维归一化，需要将 (B, C, H, W) 转换为 (B, H, W, C)
                skip_pre_norm_processed = skip_aligned.permute(0, 2, 3, 1) # (B, H, W, C)
                skip_pre_norm_processed = self.pre_norm_layer(skip_pre_norm_processed) # 对 C 维度归一化
                skip_pre_norm_processed = skip_pre_norm_processed.permute(0, 3, 1, 2) # (B, C, H, W)
                
                # 应用 Pre-Norm RWKV 模块
                rwkv_output_pre_norm = self.rwkv_pre_norm(skip_pre_norm_processed)

                # --- 4. Post-Norm 分支处理 ---
                # Post-Norm: 先输入到 RWKV，再进行归一化
                rwkv_output_post_norm = self.rwkv_post_norm(skip_aligned) # 原始对齐后的 skip 输入
                
                # 应用 Post-Norm LayerNorm
                rwkv_output_post_norm_processed = rwkv_output_post_norm.permute(0, 2, 3, 1) # (B, H, W, C)
                rwkv_output_post_norm_processed = self.post_norm_layer(rwkv_output_post_norm_processed)
                rwkv_output_post_norm_processed = rwkv_output_post_norm_processed.permute(0, 3, 1, 2) # (B, C, H, W)

                # --- 5. 残差相加 ---
                # 将原始对齐后的 skip_aligned 与两个分支的输出进行残差连接
                # 确保所有张量 (skip_aligned, rwkv_output_pre_norm, rwkv_output_post_norm_processed)
                # 具有相同的形状 (B, C, H, W)
                skip_final_processed = skip_aligned + rwkv_output_pre_norm + rwkv_output_post_norm_processed
            else:
                # 如果不满足 RWKV 的条件（例如 T 超过 T_MAX），则不应用 RWKV
                # 直接使用对齐后的 skip_aligned
                skip_final_processed = skip_aligned
                # if self.rwkv_pre_norm is not None:
                #     print(f"Skipping RWKV for skip of shape {skip_aligned.shape} (T={current_T}) because T > T_MAX_RWKV ({self.rwkv_t_max})")
                # else:
                #     print(f"Skipping RWKV for skip of shape {skip_aligned.shape} because self.rwkv modules are not initialized.")

            # --- 6. 拼接并进行卷积 ---
            x = torch.cat([x, skip_final_processed], dim=1) # 沿着通道维度拼接
        
        else: # 如果 skip is None (通常只发生在 U-Net 的最深层解码器)
            # 这种情况下 x 保持不变，无需拼接
            pass 
